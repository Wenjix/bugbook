...
final class BrowserFeatureTests: XCTestCase {
    ...
    func testLegacyBrowserPaneContentDecodeDefaultsMissingPageZoom() throws {
        let content = PaneContent.browserDocument(urlString: "https://example.com", title: "Example")
        let data = try JSONEncoder().encode(content)
        var object = try XCTUnwrap(JSONSerialization.jsonObject(with: data) as? [String: Any])
        var openFile = try XCTUnwrap(object["openFile"] as? [String: Any])
        openFile.removeValue(forKey: "browserPageZoom")
        object["openFile"] = openFile
        let legacyData = try JSONSerialization.data(withJSONObject: object)

        let decoded = try JSONDecoder().decode(PaneContent.self, from: legacyData)

        guard case .document(let decodedFile) = decoded else {
            XCTFail("Expected a document pane.")
            return
        }
        XCTAssertEqual(decodedFile.browserPageZoom, 0.85)
        XCTAssertEqual(decodedFile.path, "https://example.com")
    }
    ...
    func testPaneItemTitlesUseResolvedContentNames() throws {
        ...
        XCTAssertEqual(PaneContent.browserDocument(urlString: "bugbook://browser", title: "New Tab").paneItemTitle, "Browser")
