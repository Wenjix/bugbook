...
import DahsoCore
import Darwin
@testable import DahsoCLI

final class DahsoCLITests: XCTestCase {
    func testPageCommandsAndBacklinksSmoke() throws {
        ...
        _ = try runJSON(
            Page.Create.parseAsRoot([
                ...
                See [[Dahso Strategy]] and #strategy.
                """)
            ])
        )
        ...
        XCTAssertEqual(appended["title"] as? String, "Dahso Strategy")
    ...
    func testPageFormatCommonMarkRemovesDahsoCommentSyntax() throws {
        let workspace = try makeWorkspace()
        let blockID = "11111111-1111-1111-1111-111111111111"
        ...
        _ = try runJSON(
            Page.Create.parseAsRoot([
                ...
                Right column
                <!-- /columns -->
                <!-- database: /tmp/Dahso/databases/Test Board -->
                """)
            ])
        )
        ...
        XCTAssertTrue(raw.contains("**Dahso database:** Test Board"))
    }
    ...
    func testPageFormatDahsoMatchesCompactShortcut() throws {
        let workspace = try makeWorkspace()
        ...
        _ = try runJSON(
            Page.Create.parseAsRoot([
                "--workspace", workspace,
                "Format Dahso",
                "--content-file", try writeTempFile(in: workspace, name: "format-dahso.md", contents: """
                # Format Dahso
        ...
        XCTAssertEqual(formatted["format_style"] as? String, "dahso")
    ...
    func testInstallCommandCanSymlinkRelativeSourcePath() throws {
        let workspace = try makeWorkspace()
        let source = try writeTempFile(in: workspace, name: "DahsoCLI-bin", contents: "binary")
        ...
        let output = try installDahsoBinary(
            sourcePath: ".tmp/DahsoCLI-bin",
    ...
    func testInstallCommandReplacesBrokenSymlinkWhenForced() throws {
        ...
        let destination = normalizePath((destinationDirectory as NSString).appendingPathComponent("dahso"))
    ...
    func testDatabaseMoveDryRunPreviewsPageReparent() throws {
        ...
        XCTAssertEqual(preview["old_relative_path"] as? String, "Dahso Team/Agent Tickets")
        ...
        XCTAssertTrue(pageRaw.contains("Dahso Team/Agent Tickets"))
    ...
    func testDatabaseMoveCanReparentUnderPageAndRetargetEmbeds() throws {
        ...
        XCTAssertEqual(moved["old_relative_path"] as? String, "Dahso Team/Agent Tickets")
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: (workspace as NSString).appendingPathComponent("Dahso Team")))
        ...
        XCTAssertFalse(pageRaw.contains("Dahso Team/Agent Tickets"))
    ...
    func testDatabaseMoveRetargetsEmbedsInsideMovedRowFiles() throws {
        ...
        XCTAssertFalse(body.contains("Dahso Team/Agent Tickets"))
    }
    ...
    func testSharedAgentsTemplateCoversNotesBoardsSkillsAndTracking() {
        let template = AgentWorkspaceTemplate.agentsMarkdown(workspace: "/tmp/Dahso")
        XCTAssertTrue(template.contains("## Notes And Pages"))
        XCTAssertTrue(template.contains("dahso page get \"Dahso Strategy\" --raw"))
        XCTAssertTrue(template.contains("dahso page headings \"Dahso Strategy\""))
        XCTAssertTrue(template.contains("dahso page format \"Dahso Strategy\" --style commonmark"))
        XCTAssertTrue(template.contains("dahso page get \"Dahso Strategy\" --section-line 110"))
        XCTAssertTrue(template.contains("--dry-run"))
        XCTAssertTrue(template.contains("dahso board create"))
...
private func makeWorkspace() throws -> String {
    let workspaceURL = FileManager.default.temporaryDirectory.appendingPathComponent("DahsoCLITests-\(UUID().uuidString)")
