...
# Captures diagnostics when Dahso appears to be spinning (high sustained CPU).
#
# Usage:
#   ./scripts/capture-dahso-spin.sh
# Optional env vars:
#   APP_MATCH=Dahso
#   CPU_THRESHOLD=70
...
#   INTERVAL=1
#   TIMEOUT=180
#   OUT_DIR="$HOME/Desktop/dahso-spin-diagnostics"

APP_MATCH="${APP_MATCH:-Dahso}"
CPU_THRESHOLD="${CPU_THRESHOLD:-70}"
CONSECUTIVE_SAMPLES="${CONSECUTIVE_SAMPLES:-4}"
INTERVAL="${INTERVAL:-1}"
TIMEOUT="${TIMEOUT:-180}"
OUT_DIR="${OUT_DIR:-$HOME/Desktop/dahso-spin-diagnostics}"
...
while ((SECONDS < deadline)); do
    ...
    if ((hits >= CONSECUTIVE_SAMPLES)); then
        ...
        log show --style compact --last 3m --predicate 'process == "Dahso" OR process == "dahso"' > "${base}.log.txt" 2>/dev/null || true
