...
Claude owns verification. Codex never touches Bugbook, MCP, or computer-use. Claude agents can use worktrees and skills.
...
- If cwd is inside `/Code/bugbook` → **Bugbook** project. Add `--filter "Project!=Canopy"` to all ticket queries.
- If cwd is inside `/Code/canopy-menu` or `/canopy` → **Canopy** project. Add `--filter "Project=Canopy"` to all ticket queries.
- Otherwise → no filter.

Apply this to every `bugbook query` command in all phases below.
...
### Query all tickets
```bash
bugbook query "Agent Tickets" --filter "Status=To Do" --body
bugbook query "Agent Tickets" --filter "Status=In Progress" --body
```
...
Steps: Launch Bugbook → navigate from the real entry point to X → do Y → capture screenshot
...
```
Eval type: cli-only
Steps: swift build && bugbook <command> ...
...
Codex gets a fully self-contained prompt. It has no access to the conversation, Bugbook, or MCP.
...
## Constraints
- You do not have access to Bugbook, MCP, or computer-use.
...
- If you create a new `.swift` file, check whether `macos/Bugbook.xcodeproj/project.pbxproj` must be updated. If you are unsure, say so explicitly.
...
3. If creating new `.swift` files, update `macos/Bugbook.xcodeproj/project.pbxproj` if needed.
...
```
1. pkill -f "debug/Bugbook" || true
2. swift build
3. .build/arm64-apple-macosx/debug/Bugbook &
4. sleep 3
...
```bash
bugbook query "Agent Tickets" --filter "Status=To Do" | jq '.total_count'
```
...
8. **Claude verifies everything.** Codex never self-verifies via MCP or Bugbook.
...
bugbook query "Agent Tickets" --filter "Status=To Do" --body
bugbook query "Agent Tickets" --filter "Status=In Progress" --body
bugbook get "Agent Tickets" <row_id> --body
bugbook update "Agent Tickets" <row_id> --set "Status=Review"
echo "<body>" | bugbook update "Agent Tickets" <row_id> --body-file -
