...
Find "Agent Tickets", then `mkdir -p ~/.bugbook` and write the cache:
```json
{
...
Check if there's a relevant Bugbook project context page:
```bash
bugbook search "<relevant project or feature area>"
```
...
**Fallback if `max` plugin is not installed:** do a passive review — show the spec and ask "does this capture what you want?". Note in the terminal that installing `max` from the `max4c/bugbook` marketplace would give active grilling with ambiguity scoring.
...
```bash
bugbook update "Agent Tickets" <row_id> --set "Project=<project_row_id>"
```
...
**Discoveries:** <Anything unexpected found during implementation that future agents should know. If nothing surprising, write "None." If something was discovered, this also gets written to the Bugbook project context page — see Knowledge Loop below.>
...
If the Discoveries field has anything meaningful (not "None"), AND there's a linked project context page in Bugbook, append the discovery:
...
```bash
bugbook db list                                    # List databases
bugbook create "<db>" --set "K=V" --body-file -    # Create row
bugbook update "<db>" <id> --set "K=V"             # Update properties
bugbook update "<db>" <id> --body-file -           # Update body
bugbook get "<db>" <id> --body                     # Get row with body
bugbook query "<db>" --filter "K=V" --body         # Query with filters
bugbook page update "<page>" --section "X" --create-section --content-file -  # Update page section
bugbook context "<page>" --depth 2                 # Read page with linked context
bugbook search "<query>"                           # Search across Bugbook
```
