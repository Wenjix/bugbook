...
Start-of-session orientation. Bring the user up to speed by writing a **Session Brief** page in Bugbook. The user reads the brief in the app — visual, structured, with embedded database views they can click into.

The terminal gets a short summary. Bugbook gets the real content.
...
- `/Code/bugbook` → add `--filter "Project!=Canopy"` to all ticket queries
- `/Code/canopy-menu` or `/canopy` → add `--filter "Project=Canopy"` to all ticket queries
- Otherwise → no filter

Apply this to every `bugbook query` command below.

---

## Output: Session Brief Page

Every /catchup creates or updates a page called **"Session Brief"** in the Bugbook workspace. Use section-level updates so each run refreshes the content without losing structure.

```bash
# Create the page if it doesn't exist, or update if it does
bugbook page create "Session Brief" --title "Session Brief" --content-file - <<< ""
...
### Ticket counts
```bash
bugbook query "Agent Tickets" --filter "Status=Review"
bugbook query "Agent Tickets" --filter "Status=In Progress"
bugbook query "Agent Tickets" --filter "Status=To Do"
bugbook query "Agent Tickets" --filter "Status=Done"
```
...
For each active project, read its Bugbook context page and look for discoveries dated since the last session:
...
Build the page content and write it to Bugbook. Use `--section` with `--create-section` to update each section independently.

### Local Changes section

```bash
cat <<'CONTENT' | bugbook page update "Session Brief" --section "Local Changes" --create-section --content-file -
<based on git status output>

If uncommitted changes exist:
  **3 files modified** (uncommitted):
  - `Sources/Bugbook/Views/Editor/BlockTextView.swift` (+42, -8)
  - `Sources/Bugbook/Lib/AttributedStringConverter.swift` (+31, -2)
  - `Sources/Bugbook/Views/Editor/FlashcardReviewView.swift` (+18, -3)
...
```bash
cat <<'CONTENT' | bugbook page update "Session Brief" --section "Overnight Results" --create-section --content-file -
...
```bash
cat <<'CONTENT' | bugbook page update "Session Brief" --section "Discoveries" --create-section --content-file -
...
These have been written to each project's context page in Bugbook. Future agents will see them automatically.
CONTENT
```
...
```bash
cat <<'CONTENT' | bugbook page update "Session Brief" --section "Project Snapshot" --create-section --content-file -
| Status | Count |
|--------|-------|
| Done (this week) | 8 |
| In Review | 6 |
...
```bash
cat <<'CONTENT' | bugbook page update "Session Brief" --section "Tickets in Review" --create-section --content-file -
...
**To test:** `git checkout dev` → Open Xcode → `macos/Bugbook.xcodeproj` → Cmd+R to build and run.
...
Mark each ticket Done in Bugbook after testing, or write feedback and set back to To Do.
CONTENT
```
...
Embed the Agent Tickets database so the user can also browse in Bugbook:

```bash
bugbook page embed-database "Session Brief" "Agent Tickets"
```
...
```bash
cat <<'CONTENT' | bugbook page update "Session Brief" --section "What Needs Attention" --create-section --content-file -
...
```bash
cat <<'CONTENT' | bugbook page update "Session Brief" --section "Suggested Next Steps" --create-section --content-file -
...
```
Session Brief updated in Bugbook.
...
Smoke test checklist is in the Session Brief page in Bugbook.

Walk through reviews here, or test from Xcode and mark tickets Done in Bugbook?
```
...
If the user wants to review branches in the terminal (rather than just in Bugbook), walk through each one:
...
- **Accept** — Merge to dev, clean up worktree, set Done
  ...
  bugbook update "Agent Tickets" <row_id> --set "Status=Done"
  ```
...
- **Iterate** — Keep branch, formalize feedback as structured failure context
  ...
  **Constraints for next attempt:** <specific, actionable>
  ...
  bugbook update "Agent Tickets" <row_id> --set "Status=To Do"
  ```
...
- **"show ticket"** — full Bugbook ticket body
...
```bash
# Bugbook pages
bugbook page create "Session Brief" --content-file -
bugbook page update "Session Brief" --section "<heading>" --create-section --content-file -
bugbook page get "Session Brief" --raw
bugbook page embed-database "Session Brief" "Agent Tickets"
bugbook context "<page>" --depth 2
