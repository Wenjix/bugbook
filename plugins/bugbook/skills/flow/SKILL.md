...
Use the `book` skill for Bugbook CLI patterns and the `swiftui-pro` skill for SwiftUI conventions when working in Swift code.
...
```bash
bugbook query "Agent Projects" --filter "Status=Active"
```
...
- If cwd is inside `/Code/bugbook` → **Bugbook** project. Exclude Canopy tickets from queries. When creating, don't set Project (Bugbook is the default/home project).
...
**Query patterns:**
```bash
# Bugbook context — exclude Canopy
bugbook query "Agent Tickets" --filter "Status=To Do" --filter "Project!=Canopy" --body
...
**Cache file:** `~/.bugbook/flow-cache.json`
...
**Logic:**
1. Check if `~/.bugbook/flow-cache.json` exists. If yes, read it.
2. If missing or a CLI call fails, run `bugbook db list`, find "Agent Projects" and "Agent Tickets", then `mkdir -p ~/.bugbook` and write the cache.
...
Before creating any tickets, write a proper spec. The spec lives as a Bugbook project context page that all tickets reference and agents consult during execution. It's a living document — updated as implementation reveals new information.

This phase delegates the heavy lifting (codebase exploration, Socratic interview, spec drafting, ambiguity gate) to the `max:write-prd` skill from the companion general skills plugin. /flow handles the Bugbook-specific parts around it: pulling prior context before invocation, and persisting the result to a Bugbook project context page after.

### 2a. Pull existing Bugbook context

Check if there's already relevant context in Bugbook:
```bash
bugbook search "<project area or feature name>"
```

If a context page exists, read it:
```bash
bugbook context "<ProjectPage>" --depth 2
```
...
The `## Discoveries` section is Bugbook-specific — it's the knowledge loop that feeds discoveries back from execution to future planning. `max:write-prd` doesn't know about this; /flow adds it on persistence.

If `max:write-prd` already appended an `## Ambiguity Report` section (spec-mode grill does this), preserve it in the Bugbook page.
...
```bash
cat <<'SPEC' | bugbook create "Agent Tickets" \
...
Each `bugbook create` returns `{"created": true, "id": "row_xxx"}`. Save every row ID.
...
bugbook update "Agent Tickets" <row_a> --set "Linked=<row_b>"
bugbook update "Agent Tickets" <row_b> --set "Linked=<row_a>"

# If ticket A shares files with both B and C
bugbook update "Agent Tickets" <row_a> --set "Linked=<row_b>,<row_c>"
```
...
Wait for the user to approve. They may edit, add, remove, or reprioritize tickets in the Bugbook app. When they give the go-ahead, proceed.
...
Re-query To Do tickets from Bugbook — the user may have changed things during the review:

```bash
bugbook query "Agent Tickets" --filter "Project=<project_row_id>" --filter "Status=To Do" --body
```
...
### 1. Set In Progress
```bash
bugbook update "Agent Tickets" <row_id> --set "Status=In Progress"
```
...
```bash
# Get current body
bugbook get "Agent Tickets" <row_id> --body | jq -r '.body // ""' > /tmp/ticket_body.md
...
# Write back
cat /tmp/ticket_body.md | bugbook update "Agent Tickets" <row_id> --body-file -
```

**Knowledge Loop:** If Discoveries has anything meaningful (not "None"), append it to the project context page:

```bash
bugbook page update "<ProjectPage>" --section "Discoveries" --create-section --content-file - << EOF
...
### 6. Set to Review
```bash
bugbook update "Agent Tickets" <row_id> --set "Status=Review"
```
...
xcodebuild -project macos/Bugbook.xcodeproj -scheme BugbookApp -configuration Debug build 2>&1 | tail -3

# Launch if not running
open /Users/maxforsey/Library/Developer/Xcode/DerivedData/Bugbook-dgmanoqdbbqekudyzmycpiaxegkk/Build/Products/Debug/Bugbook.app
sleep 3

# Navigate to the relevant page/view
osascript -e '
tell application "Bugbook" to activate
delay 0.3
...
# Capture screenshot
WID=$(swift -e 'import Cocoa; let o: CGWindowListOption = [.optionOnScreenOnly, .excludeDesktopElements]; guard let wl = CGWindowListCopyWindowInfo(o, kCGNullWindowID) as? [[String: Any]] else { exit(1) }; for w in wl { if (w["kCGWindowOwnerName"] as? String ?? "").contains("Bugbook") && (w["kCGWindowLayer"] as? Int ?? -1) == 0 { print(w["kCGWindowNumber"] as? Int ?? 0); break } }')
...
```bash
bugbook query "Agent Tickets" --filter "Project=<project_row_id>" --filter "Status=Done"
```
...
# Get schema
bugbook db schema "<db_name_or_id>"

# Create row
echo "<body>" | bugbook create "<db>" --set "Key=Value" --body-file -

# Update row properties
bugbook update "<db>" <row_id> --set "Key=Value"

# Update row body
echo "<new body>" | bugbook update "<db>" <row_id> --body-file -

# Get row with body
bugbook get "<db>" <row_id> --body

# Query with filters
bugbook query "<db>" --filter "Status=To Do" --filter "Project=<row_id>" --body

# Delete row
bugbook delete "<db>" <row_id>

# Page operations
bugbook page create "<name>" --content-file -
bugbook page update "<name>" --section "X" --create-section --content-file -
bugbook page get "<name>" --raw
bugbook context "<name>" --depth 2
bugbook search "<query>"
```
