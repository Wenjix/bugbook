...
2. **Wikilinks are resilient to moves** — Bugbook resolves by filename, not path. But verify after moving.
...
Read the workspace via CLI. Do not try to read 400 files sequentially — use `bugbook page list` for metadata and selective reads for content.

### Get all pages

```bash
bugbook page list --format json
```
...
bugbook page get "<page-name>" --raw | head -30
```
...
```bash
echo '<generated index content>' | bugbook page create "Index" --content-file -
# Or update if Index already exists:
echo '<generated index content>' | bugbook page update "Index" --content-file -
```
...
```bash
echo '<summary content>' | bugbook page create "Learning/Learning" --content-file -
```
...
```bash
bugbook page get "Index" --raw | head -40
```
...
1. Detect new pages not yet in the index (compare `bugbook page list` against `Index.md` contents)
