...
description: One-shot migration from an Obsidian vault into the canonical Dahso workspace. Preserves folder hierarchy, wikilinks, and images, transforming Obsidian-specific syntax into Dahso equivalents. Use when the user says "import obsidian", "migrate my vault", "import my notes from obsidian", or pastes an Obsidian vault path. Runs phases: pre-flight → inventory → transform plan (approval gate) → write → verify → report.
---

# Import Obsidian

One-shot migration from an Obsidian vault into Dahso. The goal is to land a clean, agent-crawlable wiki in the canonical Dahso workspace without losing content, wikilinks, or media.

The user gets a dry-run preview first. Nothing is written until they approve.

**Reference spec:** `references/transforms.md` in this skill folder — cites the exact Obsidian → Dahso transform rules. Read it before writing code that touches content.
...
Always the canonical Dahso workspace. On the user's Mac, `~/Documents/Dahso` is a symlink into the iCloud container and is the correct target. Do **not** ask the user to pick — `WorkspaceResolver` is the source of truth.

```bash
DEST="$HOME/Documents/Dahso"
...
Dahso wikilinks resolve by filename minus `.md`, case-insensitive, across all folders. Two files named `Note.md` in different folders both collide on `[[Note]]`. Detect them now.
...
Dahso derives the page title from the first H1. Pages without one will get an H1 synthesized from the filename.
...
```bash
dahso page get "<some-page>" --raw | head -40
