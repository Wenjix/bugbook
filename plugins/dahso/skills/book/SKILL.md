---
name: bb
description: Access your dahso workspace — local-first notes, databases, and personal context. Use when the user wants to see their notes, tickets, databases, or personal knowledge.
---
...
Dahso stores workspaces in these locations (check in order):

1. **iCloud (macOS/iOS):** `~/Library/Mobile Documents/iCloud~com~dahso~app/Documents/`
2. **Local fallback:** `~/Documents/Dahso/`

To find your workspace:
```bash
ls ~/Library/Mobile\ Documents/iCloud~com~dahso~app/Documents/ 2>/dev/null || ls ~/Documents/Dahso/
```
...
Dahso pages are composed of these block types, each with its markdown syntax:
...
| Database embed | `<!-- dahso-database: path -->` | Embeds a database view inline |
| Page link | `<!-- dahso-page-link: Name -->` | Link to a sub-page |
| Columns | `<!-- dahso-columns -->` | Multi-column layout |
...
Use the `dahso` CLI for surgical edits. Always prefer the most targeted command — don't rewrite a whole page to change one block.

### Find blocks on a page

```bash
# List all blocks with their IDs and types
dahso block list "Page Name"

# Get a specific block by selector
dahso block get "Page Name" path:0
dahso block get "Page Name" <block-uuid>
```
...
New paragraph text here.' | dahso block replace "Page Name" <block-selector> --content-file -
```
...
echo '- [ ] New action item' | dahso block insert "Page Name" <block-selector> --after --content-file -

# Insert before a specific block
echo '---' | dahso block insert "Page Name" <block-selector> --before --content-file -
```
...
echo 'Updated bullet text' | dahso block update-text "Page Name" <block-selector> --text-file -
```
...
echo 'New section content here.' | dahso page update "Page Name" --section "Section Title" --content-file -

# Append to a section
echo '- New item' | dahso page update "Page Name" --section "Action Items" --append-file -

# Create a section if it doesn't exist
echo '- First item' | dahso page update "Page Name" --section "Notes" --create-section --section-level 2 --content-file -
```
...
echo 'Changed text' | dahso block replace "Page Name" <block-selector> --content-file - --dry-run
```

## Database Row Editing

Use the `dahso` CLI for database CRUD. The `create`, `update`, `get`, `query`, and `delete` commands work on database rows.
...
dahso db schema "Database Name"

# Query rows with filters
dahso query "Database Name" --filter "Status=To Do" --body

# Get a single row with body
dahso get "Database Name" <row_id> --body
```
...
Additional details.' | dahso create "Database Name" \
  --set "Name=Row Title" \
  --set "Status=To Do" \
  --set "Priority=High" \
  --body-file -
...
dahso update "Database Name" <row_id> --set "Status=Done"
dahso update "Database Name" <row_id> --set "Status=In Progress" --set "Priority=High"
```
...
New body here.' | dahso update "Database Name" <row_id> --body-file -
```

### Delete a row

```bash
dahso delete "Database Name" <row_id>
```
...
Content here.' | dahso page create "Page Name" --content-file -
```
...
```bash
# Full page as JSON
dahso page get "Page Name"

# Raw markdown only
dahso page get "Page Name" --raw

# Just a section
dahso page get "Page Name" --section "Section Title"

# With parsed block data
dahso page get "Page Name" --blocks
```
...
```bash
dahso context "Page Name" --depth 2
```
...
When working on tickets in `Dahso Team/Tickets/`:
