...
The active Dahso workspace is **Dahso 2** in iCloud. All `dahso` commands need the workspace flag:

```bash
WS="$HOME/Library/Mobile Documents/iCloud~com~dahso~app/Documents/Dahso 2"
dahso query "Agent Tickets" --workspace "$WS"
```

Set `WS` once at the start of the session and use it everywhere. The symlink at `~/Documents/Dahso/` points to a different (older) workspace — do not use the default.
...
```bash
dahso db schema "Agent Tickets" --workspace "$WS" 2>/dev/null
dahso db schema "Max Tickets" --workspace "$WS" 2>/dev/null
dahso db schema "Weekly Reviews" --workspace "$WS" 2>/dev/null
```

Check for the previous week's review to compare against:
```bash
dahso query "Weekly Reviews" --workspace "$WS" 2>/dev/null
```
...
```bash
echo '<filled template>' | dahso create "Weekly Reviews" \
...
Keep terminal output concise — the rich content is in Dahso:
...
```bash
dahso query "Max Tickets" --workspace "$WS" --filter "Status=To Do"
dahso query "Agent Tickets" --workspace "$WS" --filter "Status=To Do"
```
...
```bash
dahso context "Alignment Zone" --workspace "$WS" --depth 1
```
...
Open in Dahso to see the full page.
```
...
- Terminal output stays concise. Rich content goes to Dahso.
