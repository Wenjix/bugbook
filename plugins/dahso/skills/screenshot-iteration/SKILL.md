...
pkill -f "debug/Dahso" 2>/dev/null || true
swift build 2>&1 | tail -10
.build/arm64-apple-macosx/debug/Dahso &
sleep 3
```
...
# Or full window:
screencapture -x -l$(osascript -e 'tell app "Dahso" to id of window 1') .screenshot-iteration/round-0/<state>.png
```
...
screencapture -x -l$(osascript -e 'tell app "Dahso" to id of window 1') .screenshot-iteration/round-$N/<state>.png
```
...
Getting the app to the target state is the hardest part of this skill because Dahso has no test harness yet. Three tiers, use the simplest that works:
...
```bash
osascript -e 'tell application "Dahso" to activate'
...
mcp__computer-use__request_access(
  apps: ["Dahso"],
  reason: "Drive Dahso to target UI state for visual iteration",
...
7. **Clean up background Dahso processes.** `pkill -f "debug/Dahso"` before every launch so stale instances don't eat screenshots.
