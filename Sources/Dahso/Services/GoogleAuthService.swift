...
private final class GoogleOAuthLoopbackServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "Dahso.GoogleOAuthLoopback")
    private let lock = NSLock()
    ...
    private func receiveRequest(on connection: NWConnection, data: Data) {
        connection.receive(minimumIncompleteLength: 1, maximumLength: 8192) { [weak self] chunk, _, isComplete, error in
            ...
            do {
                ...
                <html><body><p>You can close this window and return to Dahso.</p><script>window.close()</script></body></html>
                """)
            } catch {
                result = .failure(error)
                self.respond(on: connection, body: """
                <html><body><p>Dahso sign-in failed. You can close this window and return to the app.</p><script>window.close()</script></body></html>
                """)
            }
