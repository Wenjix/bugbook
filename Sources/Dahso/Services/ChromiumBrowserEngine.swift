...
enum ChromiumRuntimeMetadata {
    static func productVersion() -> String? {
        let description = BBChromiumRuntime.runtimeDescription()
        guard let range = description.range(of: #"chromium-\d+\.\d+\.\d+\.\d+"#, options: .regularExpression) else {
            return nil
        }
        return String(description[range]).replacingOccurrences(of: "chromium-", with: "")
    }
}
...
final class ChromiumBrowserEngine: BrowserEngine {
    ...
    func configureExtensions(_ extensionPaths: [String]) {
        BBChromiumRuntime.configureExtensionPaths(extensionPaths)
    }
...
enum ChromiumRuntimeMetadata {
    static func productVersion() -> String? { nil }
}
#endif
