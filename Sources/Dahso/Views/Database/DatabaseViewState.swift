...
final class DatabaseViewState {
    ...
    func createRow() throws -> DatabaseRow {
        guard let s = schema else {
            throw NSError(domain: "Dahso.Database", code: 1, userInfo: [NSLocalizedDescriptionKey: "Schema unavailable"])
        }
    ...
    func addView(type: ViewType) {
        ...
        if type == .calendar {
            calendarDatePropertyId = try? Dahso.ensureCalendarDateProperty(
    ...
    func ensureCalendarDateProperty(preferredPropertyId: String?) throws -> (DatabaseSchema, String) {
        ...
        let datePropertyId = try Dahso.ensureCalendarDateProperty(
