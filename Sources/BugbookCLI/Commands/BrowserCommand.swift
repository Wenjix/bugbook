...
struct Browser: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "browser",
        abstract: "Inspect Bugbook browser tabs, history, saved pages, and persisted content",
        subcommands: [Tabs.self, History.self, Saved.self, Content.self]
    )

    struct StoreOptions: ParsableArguments {
        @Option(name: .long, help: "Bugbook app support directory. Defaults to ~/Library/Application Support/Bugbook.")
        ...
        var appSupportURL: URL {
            ...
            return base.appendingPathComponent("Bugbook", isDirectory: true)
        }
    }
    ...
    struct Tabs: ParsableCommand {
        static let configuration = CommandConfiguration(
            commandName: "tabs",
            abstract: "List Bugbook browser tabs from persisted workspace layouts and pane snapshots"
        )
    ...
    struct History: ParsableCommand {
        static let configuration = CommandConfiguration(
            commandName: "history",
            abstract: "List recent Bugbook browser history entries"
        )
...
private extension Dictionary where Key == String, Value == Any {
    ...
    var browserURLString: String {
        guard let path = string("path"), path != "bugbook://browser" else {
            return ""
        }
        return path
    }
