...
final class AgentHubViewModel {
    ...
    func start(workspacePath: String?) {
        SentryBreadcrumbs.add(Breadcrumb(level: .info, category: "agent.start"))
        stop()
    ...
    func refresh(workspacePath: String?) {
        ...
        do {
            ...
            SentryBreadcrumbs.add(Breadcrumb(level: .error, category: "agent.error"))
    ...
    func createTask(workspacePath: String?, title: String, assignee: String?) {
        ...
        do {
            ...
            SentryBreadcrumbs.add(Breadcrumb(level: .info, category: "agent.create"))
            refresh(workspacePath: workspacePath)
        } catch {
            SentryBreadcrumbs.add(Breadcrumb(level: .error, category: "agent.error"))
            self.error = error.localizedDescription
