...
class MeetingNoteService {
    ...
    func createOrOpenMeetingNote(
        ...
        if let fileSystem {
            let body = MarkdownBlockParser.stripYAMLFrontmatter(content).content
            let duration = event.isAllDay ? nil : Int(event.endDate.timeIntervalSince(event.startDate) / 60)
            let attendees = event.attendees.map { $0.displayName ?? $0.email }
            do {
                let location = try await fileSystem.createMeetingDatabaseRowInBackground(
                    in: workspace,
                    title: event.title,
                ...
                if let rowPath = location.rowPath {
                    try eventStore.linkEventToPage(eventId: event.id, pagePath: rowPath, in: workspace)
    ...
    func importRecording(
        ...
        do {
            ...
            try await Self.writeContent(content, to: pagePath)
            return pagePath
        } catch {
            self.error = error.localizedDescription
            return nil
    ...
    private nonisolated static func writeContent(_ content: String, to path: String) async throws {
    ...
    func createAdHocMeetingPage(
        ...
        if let fileSystem {
            do {
                let location = try fileSystem.createMeetingDatabaseRow(
                    in: workspace,
                    title: title,
                    date: date,
                    body: "# \n"
    ...
    func createAdHocMeetingPageAsync(
        title: String,
        date: Date,
        workspace: String,
        fileSystem: FileSystemService? = nil
        ...
        do {
            return try await Self.writeAdHocMeetingPage(title: title, date: date, workspace: workspace)
        } catch {
            self.error = error.localizedDescription
            return nil
        }
    ...
    nonisolated static func yamlEscape(_ value: String) -> String {
    ...
    private nonisolated static func writeAdHocMeetingPage(
        title: String,
        date: Date,
        workspace: String
    ) async throws -> String? {
        try await Task.detached(priority: .utility) {
            ...
            let baseFilename = sanitize(title)
            let baseName = "\(dateStr) — \(baseFilename)"

            var pagePath = (workspace as NSString).appendingPathComponent("\(baseName).md")
            var counter = 2
            while FileManager.default.fileExists(atPath: pagePath) && counter <= 999 {
                pagePath = (workspace as NSString).appendingPathComponent("\(baseName) \(counter).md")
                counter += 1
            }
            guard !FileManager.default.fileExists(atPath: pagePath) else { return nil }
