...
class BlockDocument {
    ...
    /// Whether the live meeting capture flow has finalized for this meeting page.
    var isCompletedMeetingPage: Bool {
        guard isMeetingPage else { return false }
        return MarkdownBlockParser.yamlValue(for: "recorded_at", in: yamlFrontmatter) != nil
    }
    ...
    func block(for id: UUID) -> Block? {
        for block in blocks {
            ...
            if block.type == .column || block.type == .toggle || block.type == .headingToggle || block.type == .meeting {
    ...
    func changeBlockType(id: UUID, to type: BlockType) {
        ...
        updateBlockProperty(id: id) { block in
            ...
            if type == .footnote, block.footnoteLabel.isEmpty {
                block.footnoteLabel = nextFootnoteLabel()
            }
        }
    }

    private func nextFootnoteLabel() -> String {
        let labels: Set<String> = Set(flattenBlocks(blocks).compactMap { block in
            guard block.type == .footnote, !block.footnoteLabel.isEmpty else { return nil }
            return block.footnoteLabel
        })
        var candidate = 1
        while labels.contains(String(candidate)) {
            candidate += 1
        }
        return String(candidate)
    }

    private func flattenBlocks(_ blocks: [Block]) -> [Block] {
        blocks.flatMap { block in
            [block] + flattenBlocks(block.children)
        }
    }
    ...
    func duplicateBlock(id: UUID) {
        ...
        let copy = Block(
            ...
            meetingState: original.meetingState,
            meetingTranscript: original.meetingTranscript,
            meetingSummary: original.meetingSummary,
            meetingActionItems: original.meetingActionItems,
            meetingTitle: original.meetingTitle,
            meetingNotes: original.meetingNotes,
            tableData: original.tableData,
            hasHeaderRow: original.hasHeaderRow,
            calloutIcon: original.calloutIcon,
            calloutColor: original.calloutColor,
            footnoteLabel: original.footnoteLabel
        )
        if let childIdx = loc.child {
    ...
    static let slashCommands: [SlashCommand] = [
        ...
        SlashCommand(
            name: "Footnote",
            icon: "asterisk",
            action: .blockType(.footnote, headingLevel: 0),
    ...
    static var availableSlashCommands: [SlashCommand] {
        slashCommands.filter { command in
            switch command.action {
            case .askAI:
                BugbookFeatureGate.legacyPanesEnabled
            default:
                true
            }
        }
    ...
    var filteredSlashCommands: [SlashCommand] {
        if slashMenuFilter.isEmpty { return Self.availableSlashCommands }
        return Self.availableSlashCommands.filter { $0.matches(slashMenuFilter) }
    }
    ...
    func executeSlashCommand() {
        ...
        case let .blockType(type, headingLevel):
            ...
            // Footnote block — assign the next available numeric label.
            if type == .footnote {
                saveUndo()
                updateBlockProperty(id: blockId) { block in
                    block.type = .footnote
    ...
    /// Copies an image file into the workspace `_assets/` directory and returns the markdown-stored relative path.
    func copyImageToAssets(_ sourceURL: URL) -> String? {
        ...
        do {
            ...
            return relativeWorkspacePath(for: destPath, workspace: workspace)
        } catch {
            return nil
        }
    }
    ...
    /// Saves raw image data to the workspace `_assets/` directory and returns the markdown-stored relative path.
    ...
    private func relativeWorkspacePath(for absolutePath: String, workspace: String) -> String {
        let normalizedWorkspace = (workspace as NSString).standardizingPath
        let normalizedPath = (absolutePath as NSString).standardizingPath
        let prefix = normalizedWorkspace.hasSuffix("/") ? normalizedWorkspace : normalizedWorkspace + "/"
