...
struct AppSettings: Codable, Equatable {
    static let profileWorkspacePathEnvironmentKey = "BUGBOOK_PROFILE_WORKSPACE_PATH"
    ...
    /// Explicit notes folder root. Empty string = default Bugbook workspace resolution.
    ...
    func resolvedNotesFolderPath(environment: [String: String] = ProcessInfo.processInfo.environment) -> String {
        let profileWorkspacePath = Self.profileWorkspacePath(environment: environment)
        if !profileWorkspacePath.isEmpty {
            return profileWorkspacePath
        }
        return notesFolderPath.trimmingCharacters(in: .whitespacesAndNewlines)
    }

    private static func profileWorkspacePath(environment: [String: String]) -> String {
        (environment[profileWorkspacePathEnvironmentKey] ?? "")
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(notesFolderPath, forKey: .notesFolderPath)
