...
final class DatabaseViewState {
    ...
    func loadData(onLoaded: (() -> Void)? = nil) {
        ...
        loadTask = Task { [weak self] in
            ...
            let result = await Task.detached(priority: .userInitiated) { () -> Result<DatabaseService.DisplayLoadResult, Error> in
                do {
                    return .success(try service.loadDatabaseForDisplay(at: path))
                } catch {
            ...
            case .success(let loaded):
                applyLoadedDatabase(schema: loaded.schema, rows: loaded.rows, onLoaded: onLoaded)
                if loaded.needsDiskRefresh {
    ...
    private func refreshFromDiskInBackground(onLoaded: (() -> Void)?) {
        ...
        diskRefreshTask = Task { [weak self] in
            guard let self else { return }
            let result = await Task.detached(priority: .utility) { () -> Result<(DatabaseSchema, [DatabaseRow]), Error> in
                do {
                    return .success(try service.loadDatabaseFromDiskRefreshingIndex(at: path))
                } catch {
                    return .failure(error)
                }
            }.value
            ...
            case .success(let (loadedSchema, loadedRows)):
                guard pendingRowSaves.isEmpty else { return }
                applyLoadedDatabase(schema: loadedSchema, rows: loadedRows, onLoaded: onLoaded)
            case .failure:
                break
            }
        }
