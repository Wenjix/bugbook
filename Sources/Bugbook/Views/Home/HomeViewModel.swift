...
extension HomeViewModel {
    ...
    nonisolated static func freeUntil(events: [CalendarEvent], now: Date) -> String? {
        ...
        for event in events where event.endDate > now {
            if event.startDate > cursor {
                return "Free until \(shortTimeString(from: event.startDate))"
            }
            cursor = max(cursor, event.endDate)
        }
    ...
    private nonisolated static func shortTimeString(from date: Date) -> String {
    ...
    nonisolated static func agentDate(_ rawValue: String?) -> Date? {
        guard let rawValue else { return nil }
        return fractionalAgentDateFormatter().date(from: rawValue)
            ?? basicAgentDateFormatter().date(from: rawValue)
    }

    private nonisolated static func fractionalAgentDateFormatter() -> ISO8601DateFormatter {
        let f = ISO8601DateFormatter()
        f.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
        return f
    }

    private nonisolated static func basicAgentDateFormatter() -> ISO8601DateFormatter {
