...
struct BrowserSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            SettingsSection("Extensions") {
                HStack(alignment: .top, spacing: 12) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Unpacked Extensions")
                            .font(.system(size: 13, weight: .medium))
                        Text("Select an unpacked extension folder, or a Chrome extension folder that contains versioned subfolders. Changes apply the next time Bugbook launches.")
                            .font(.system(size: 12))
                            .foregroundStyle(.secondary)
                    }
                    Spacer()
                ...
                Text("Store installs are imported into Bugbook's local extension storage, then loaded on next launch.")
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
    ...
    private func installChromeWebStoreExtension() async {
        ...
        do {
            ...
            chromeWebStoreInput = ""
            extensionMessage = "Installed \(installation.displayName). Restart Bugbook to load it."
        } catch {
    ...
    private func addExtensions() {
        ...
        extensionMessage = "Restart Bugbook to load the selected extensions."
    }

    private func removeExtension(_ path: String) {
        appState.settings.browserExtensionPaths.removeAll { $0 == path }
        extensionMessage = "Restart Bugbook to unload removed extensions."
    }
...
enum BrowserExtensionStoreInstallerError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        ...
        case .runtimeUnavailable:
            return "Bugbook could not determine the embedded Chromium version for this install."
        case .invalidStoreResponse:
