...
struct SearchSettingsView: View {
    ...
    private let qmdRepositoryURL = URL(string: "https://github.com/tobi/qmd")
    ...
    private var mcpSection: some View {
        SettingsSection("MCP Server") {
            VStack(alignment: .leading, spacing: 8) {
                Text(
                    "qmd also runs as a standalone MCP server. Add it to Claude Code, Cursor, " +
                    "or any MCP-compatible client to get search across your notes outside of Bugbook."
                )
                    .font(.system(size: 13))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
                if let qmdRepositoryURL {
                    Link("github.com/tobi/qmd", destination: qmdRepositoryURL)
                        .font(.system(size: 13))
                }
            }
        }
