...
struct MeetingBlockView: View {
    ...
    private static let summaryService = MeetingSummaryService()
...
private extension MeetingBlockView {
    ...
    private var afterStateView: some View {
        ...
        return VStack(spacing: 0) {
            HStack(spacing: 10) {
                ...
                if sections.isEmpty,
                   block.meetingActionItems.isEmpty,
                   block.meetingSummary.isEmpty,
                   !hasHeadingChild,
                   !summarySourceText.isEmpty {
                    Button {
...
private extension MeetingBlockView {
    ...
    private var summarySourceText: String {
        [
            ...
            block.meetingNotes,
            block.children.map(\.text).joined(separator: "\n")
        ]
        .joined(separator: "\n")
...
private extension MeetingBlockView {
    ...
    private func generateSummary() async {
        ...
        let input = summaryInput(transcript: transcript, notes: userNotes)
        ...
        do {
            let result = try await Self.summaryService.generateSummary(
                transcript: input,
                command: AppSettings.default.meetingSummaryCommand
            )
            if let title = result.title, !title.isEmpty {
                document.updateMeetingTitle(blockId: block.id, title: title)
            }

            let summaryBlocks = markdownToBlocks(
                summaryMarkdown(from: result.summary),
                actionItems: actionItemsMarkdown(from: result.actionItems)
            )
            ...
            document.updateBlockProperty(id: block.id) { block in
                block.meetingSummary = "Summary unavailable: \(error.localizedDescription)"
            }
        }
    ...
    private func summaryInput(transcript: String, notes: String) -> String {
        [
    ...
    private func summaryMarkdown(from bullets: [String]) -> String {
        guard !bullets.isEmpty else { return "" }
        return "## Summary\n" + bullets.map { "- \($0)" }.joined(separator: "\n")
    }

    private func actionItemsMarkdown(from actionItems: [String]) -> String {
