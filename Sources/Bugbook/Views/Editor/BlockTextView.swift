...
enum BlockTextContentMode: Equatable {
    case markdown
    case plainCode(language: String)

    var usesInlineMarkdown: Bool {
        self == .markdown
    }
}
...
struct BlockTextView: NSViewRepresentable {
    ...
    var contentMode: BlockTextContentMode = .markdown
    ...
    func makeNSView(context: Context) -> BlockNSTextView {
        ...
        if let block = document.block(for: blockId) {
            let attributed = attributedText(from: block.text)
        ...
        context.coordinator.lastContentMode = contentMode
    ...
    func updateNSView(_ textView: BlockNSTextView, context: Context) {
        ...
        let contentModeChanged = contentMode != context.coordinator.lastContentMode
        if contentModeChanged {
            context.coordinator.lastContentMode = contentMode
            context.coordinator.lastTextColor = textColor
            context.coordinator.lastFont = font
            context.coordinator.rebuildTextPreservingSelection(textView)
        }

        // Re-apply foreground color and strikethrough to existing text when textColor changes
        if !contentModeChanged, textColor != context.coordinator.lastTextColor {
            context.coordinator.lastTextColor = textColor
        ...
        if !contentModeChanged, font != context.coordinator.lastFont {
            context.coordinator.lastFont = font
            let fullRange = NSRange(location: 0, length: textView.textStorage?.length ?? 0)
            if fullRange.length > 0 {
                context.coordinator.withProgrammaticViewUpdate {
    ...
    func attributedText(from text: String) -> NSAttributedString {
        switch contentMode {
        case .markdown:
            return AttributedStringConverter.attributedString(
                from: text,
                font: font,
                textColor: textColor
            )
        case .plainCode(let language):
            return CodeSyntaxHighlighter.attributedString(
                from: text,
                language: language,
                font: font,
                textColor: textColor
    ...
    class Coordinator: NSObject, NSTextViewDelegate {
        ...
        var lastFont: NSFont?
        var lastContentMode: BlockTextContentMode?
        ...
        func toggleBold() {
            guard parent.contentMode.usesInlineMarkdown else { return }
        ...
        func textView(
            ...
            if !suppressChanges,
               BugbookFeatureGate.shouldExposeAgentSurfaces,
        ...
        func textDidChange(_ notification: Notification) {
            ...
            applyCodeHighlightingIfNeeded(textView)
        ...
        private func autoDetectMarkdownPrefix(_ textView: NSTextView) {
            ...
            if let footnote = parseFootnoteDefinitionPrefix(text) {
                convertFootnoteBlock(textView, label: footnote.label, text: footnote.text)
                return
            }
        ...
        private func parseFootnoteDefinitionPrefix(_ text: String) -> (label: String, text: String)? {
            guard text.hasPrefix("[^") else { return nil }
            let labelStart = text.index(text.startIndex, offsetBy: 2)
            guard let labelEnd = text[labelStart...].firstIndex(of: "]") else { return nil }
            let colonIndex = text.index(after: labelEnd)
            guard colonIndex < text.endIndex, text[colonIndex] == ":" else { return nil }

            let label = String(text[labelStart..<labelEnd]).trimmingCharacters(in: .whitespacesAndNewlines)
            guard !label.isEmpty else { return nil }

            var textStart = text.index(after: colonIndex)
            if textStart < text.endIndex, text[textStart] == " " {
                textStart = text.index(after: textStart)
            }
            let content = textStart < text.endIndex ? String(text[textStart...]) : ""
            return (label, content)
        }

        private func convertBlock(
            _ textView: NSTextView,
            stripping prefixLen: Int,
            to type: BlockType,
            headingLevel: Int = 1,
            checked: Bool = false
        ) {
        ...
        private func convertFootnoteBlock(_ textView: NSTextView, label: String, text: String) {
            guard parent.document.blockLocation(for: parent.blockId) != nil else { return }

            parent.document.updateBlockProperty(id: parent.blockId) { block in
                block.type = .footnote
                block.footnoteLabel = label
                block.text = text
            }
        ...
        func markdownFromTextView(_ textView: NSTextView) -> String {
            guard parent.contentMode.usesInlineMarkdown else {
        ...
        private func markdownOffset(forDisplayOffset displayOffset: Int, in attributed: NSAttributedString) -> Int {
            guard parent.contentMode.usesInlineMarkdown else {
                return max(0, min(displayOffset, attributed.length))
            }
        ...
        private func displayOffset(forMarkdownOffset markdownOffset: Int, markdown: String) -> Int {
            guard parent.contentMode.usesInlineMarkdown else {
                return max(0, min(markdownOffset, (markdown as NSString).length))
            }
        ...
        func rebuildTextPreservingSelection(_ textView: NSTextView) {
            let selected = textView.selectedRange()
            let attributed = parent.attributedText(from: textView.string)
            let length = attributed.length
            let clampedLocation = min(selected.location, length)
            let clampedSelection = NSRange(
                location: clampedLocation,
                length: min(selected.length, max(0, length - clampedLocation))
            )
            withProgrammaticViewUpdate {
                textView.textStorage?.setAttributedString(attributed)
        ...
        private func applyCodeHighlightingIfNeeded(_ textView: NSTextView) {
            guard case .plainCode(let language) = parent.contentMode,
                  let storage = textView.textStorage else { return }
            let selected = textView.selectedRange()
            withProgrammaticViewUpdate {
                CodeSyntaxHighlighter.applyHighlighting(
                    to: storage,
                    language: language,
                    font: parent.font,
                    textColor: parent.textColor
