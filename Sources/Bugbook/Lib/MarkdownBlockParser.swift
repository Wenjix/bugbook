...
enum MarkdownBlockParser {
    ...
    private static func parseLines(_ lines: ArraySlice<Substring>, foundBlockIDs: inout Bool) -> [Block] {
        ...
        while i < lines.endIndex {
            ...
            // Markdown callout, e.g. `> [!NOTE] Title`.
            if let callout = parseMarkdownCalloutHeader(line) {
                i += 1
                var calloutLines: [Substring] = []
                while i < lines.endIndex {
                    guard let contentLine = stripBlockquoteMarker(String(lines[i])) else { break }
                    calloutLines.append(Substring(contentLine))
                    i += 1
                }
                let children = calloutLines.isEmpty
                    ? []
                    : parseLines(calloutLines[...], foundBlockIDs: &foundBlockIDs)
                var block = makeBlock(type: .callout, text: callout.title, children: children)
                block.calloutIcon = callout.icon
                block.calloutColor = callout.color
                blocks.append(block)
                continue
            }
            ...
            // Footnote definition
            if let footnote = parseFootnoteDefinition(line) {
                var footnoteLines = [footnote.text]
                i += 1
                while i < lines.endIndex {
                    guard let continuation = parseFootnoteContinuation(String(lines[i])) else {
                        break
                    }
                    footnoteLines.append(continuation)
                    i += 1
                }
                blocks.append(makeBlock(
                    type: .footnote,
                    text: footnoteLines.joined(separator: "\n"),
            ...
            if let headingToggleLevel = parseHeadingToggleComment(trimmed) {
                ...
                blocks.append(makeBlock(
                    type: .headingToggle,
                    text: String(title),
                    headingLevel: headingToggleLevel,
                    children: children,
            ...
            if isTableStart(line, nextLine: i + 1 < lines.endIndex ? String(lines[i + 1]) : nil) {
                ...
                while i < lines.endIndex {
                    ...
                    guard isTableDataRow(rowLine) else { break }
    ...
    private static func parseTableRow(_ line: String) -> [String] {
        let inner = trimOuterTablePipes(line.trimmingCharacters(in: .whitespaces))
    ...
    private static func trimOuterTablePipes(_ line: String) -> Substring {
        var inner = line[...]
        if inner.first == "|" {
    ...
    private static func isTableStart(_ line: String, nextLine: String?) -> Bool {
        let trimmed = line.trimmingCharacters(in: .whitespaces)
        guard isTableDataRow(trimmed) else { return false }
    ...
    private static func isTableDataRow(_ line: String) -> Bool {
        let trimmed = line.trimmingCharacters(in: .whitespaces)
        guard trimmed.contains("|") else { return false }
        return parseTableRow(trimmed).count > 1
    }
    ...
    static func serialize(_ blocks: [Block], includeBlockIDComments: Bool = false) -> String {
        ...
        for (i, block) in blocks.enumerated() {
            ...
            case .callout:
                if let calloutLines = markdownCalloutLines(for: block, includeBlockIDComments: includeBlockIDComments) {
                    lines.append(contentsOf: calloutLines)
                } else {
            ...
            case .footnote:
                let label = block.footnoteLabel.isEmpty ? "1" : block.footnoteLabel
                let footnoteLines = block.text.components(separatedBy: .newlines)
                let firstLine = footnoteLines.first ?? ""
                lines.append("[^\(label)]: \(firstLine)")
                for continuation in footnoteLines.dropFirst() {
    ...
    private static func paragraphNeedsProtection(_ line: String) -> Bool {
        ...
        if line.hasPrefix(">")
            || parseImage(line) != nil
            || parseDatabaseEmbed(line) != nil
            || parseMeetingBlock(line) != nil
            || parseWikiLink(line) != nil
            || parsePageLinkComment(line) != nil
    ...
    private static func parseTaskItem(_ line: String) -> (Int, Bool, String)? {
        ...
        guard stripped.count >= 6, let marker = stripped.first else { return nil }
    ...
    private static func parseBulletItem(_ line: String) -> (Int, String)? {
        ...
        guard stripped.count >= 2, let marker = stripped.first else { return nil }
        guard marker == "-" || marker == "*" || marker == "+" else { return nil }
        let rest = stripped.dropFirst(1)
        guard rest.hasPrefix(" ") else { return nil }
    ...
    private static func parseFootnoteDefinition(_ line: String) -> (label: String, text: String)? {
        ...
        guard let labelEnd = line[labelStart...].firstIndex(of: "]") else { return nil }
        let colonIndex = line.index(after: labelEnd)
        guard colonIndex < line.endIndex, line[colonIndex] == ":" else { return nil }

        let label = String(line[labelStart..<labelEnd]).trimmingCharacters(in: .whitespacesAndNewlines)
        guard !label.isEmpty else { return nil }

        var textStart = line.index(after: colonIndex)
    ...
    private static func parseFootnoteContinuation(_ line: String) -> String? {
        if line.hasPrefix("    ") {
    ...
    private static func parseMarkdownCalloutHeader(_ line: String) -> MarkdownCallout? {
        guard let content = stripBlockquoteMarker(line) else { return nil }
        let trimmed = content.trimmingCharacters(in: .whitespaces)
        guard trimmed.hasPrefix("[!") else { return nil }

        let kindStart = trimmed.index(trimmed.startIndex, offsetBy: 2)
        guard let kindEnd = trimmed[kindStart...].firstIndex(of: "]") else { return nil }
        let rawKind = String(trimmed[kindStart..<kindEnd])
            .trimmingCharacters(in: .whitespacesAndNewlines)
        guard !rawKind.isEmpty else { return nil }

        var titleStart = trimmed.index(after: kindEnd)
        if titleStart < trimmed.endIndex {
            let marker = trimmed[titleStart]
            if marker == "+" || marker == "-" {
                titleStart = trimmed.index(after: titleStart)
            }
        }

        let kind = normalizedMarkdownCalloutKind(rawKind)
        let presentation = markdownCalloutPresentation(for: kind)
        let title = String(trimmed[titleStart...])
            .trimmingCharacters(in: .whitespacesAndNewlines)
        return MarkdownCallout(
            kind: kind,
            title: title.isEmpty ? presentation.defaultTitle : title,
    ...
    private static func stripBlockquoteMarker(_ line: String) -> String? {
        var remainder = line.drop(while: { $0 == " " })
    ...
    private static func normalizedMarkdownCalloutKind(_ rawKind: String) -> String {
        switch rawKind.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() {
        case "hint": "TIP"
        case "caution", "attention": "WARNING"
        case "danger", "failure", "fail", "missing": "ERROR"
        case "check", "done": "SUCCESS"
        case "help", "faq": "QUESTION"
        case "abstract", "summary", "tldr": "SUMMARY"
        default: rawKind.uppercased()
        }
    }

    private static func markdownCalloutPresentation(for kind: String) -> (icon: String, color: String, defaultTitle: String) {
        switch kind {
        case "INFO":
            return ("info.circle", "blue", "Info")
        case "TIP":
            return ("lightbulb", "green", "Tip")
        case "WARNING":
            return ("exclamationmark.triangle", "orange", "Warning")
        case "SUCCESS":
            return ("checkmark.circle", "green", "Success")
        case "ERROR":
            return ("xmark.circle", "red", "Error")
        case "QUESTION":
            return ("questionmark.circle", "purple", "Question")
        case "IMPORTANT":
            return ("star", "yellow", "Important")
        case "SUMMARY":
            return ("bookmark", "blue", "Summary")
        default:
            return ("lightbulb", "default", "Note")
        }
    }

    private static func markdownCalloutLines(for block: Block, includeBlockIDComments: Bool) -> [String]? {
        guard let kind = markdownCalloutKind(icon: block.calloutIcon, color: block.calloutColor) else {
            return nil
        }
        ...
        let title = block.text
            ...
            .trimmingCharacters(in: .whitespacesAndNewlines)
        var lines = [title.isEmpty ? "> [!\(kind)]" : "> [!\(kind)] \(title)"]

        let childMarkdown = serialize(block.children, includeBlockIDComments: includeBlockIDComments)
        guard !childMarkdown.isEmpty else { return lines }

        for childLine in childMarkdown.split(separator: "\n", omittingEmptySubsequences: false) {
            lines.append(childLine.isEmpty ? ">" : "> \(childLine)")
        }
        return lines
    }

    private static func markdownCalloutKind(icon: String, color: String) -> String? {
        switch (icon, color) {
        case ("lightbulb", "default"):
            return "NOTE"
        case ("info.circle", "blue"):
            return "INFO"
        case ("lightbulb", "green"), ("bolt", "yellow"):
            return "TIP"
        case ("exclamationmark.triangle", "orange"):
            return "WARNING"
        case ("checkmark.circle", "green"):
            return "SUCCESS"
        case ("xmark.circle", "red"):
            return "ERROR"
        case ("questionmark.circle", "purple"):
            return "QUESTION"
        case ("star", "yellow"):
            return "IMPORTANT"
        case ("bookmark", "blue"):
