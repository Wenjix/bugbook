...
enum AttributedStringConverter {
    ...
    static let wikiLinkPageNameKey = NSAttributedString.Key("BugbookWikiLinkPageName")

    /// Stores the label for inline footnote reference spans.
    static let footnoteReferenceLabelKey = NSAttributedString.Key("BugbookFootnoteReferenceLabel")
    ...
    static func attributedString(
        ...
        while i < markdown.endIndex {
            ...
            if let (text, end) = parseDelimited(markdown, from: i, delimiter: "**") {
                appendBoldText(text, source: "**\(text)**", font: font, baseAttributes: baseAttributes, to: result)
                i = end
                continue
            }
            if let (text, end) = parseDelimited(markdown, from: i, delimiter: "__") {
                appendBoldText(text, source: "__\(text)__", font: font, baseAttributes: baseAttributes, to: result)
                i = end
                continue
            }
            ...
            // Inline code: `text`
            if let (text, end) = parseDelimited(markdown, from: i, delimiter: "`") {
                appendInlineCode(text, font: font, baseAttributes: baseAttributes, to: result)
                i = end
                continue
            }

            // Footnote reference: [^label] -> superscript label.
            if let (label, source, end) = parseFootnoteReference(markdown, from: i) {
                appendFootnoteReference(label, source: source, font: font, baseAttributes: baseAttributes, to: result)
                i = end
                continue
            }
            ...
            if let end = parseDoubleEqualsSeparator(markdown, from: i) {
                appendDoubleEqualsSeparator(font: font, baseAttributes: baseAttributes, to: result)
                i = end
                continue
            }
    ...
    private static func appendBoldText(
        _ text: String,
        source: String,
        font: NSFont,
    ...
    private static func appendFootnoteReference(
        _ label: String,
        source: String,
        font: NSFont,
        baseAttributes: [NSAttributedString.Key: Any],
        ...
        var attrs = baseAttributes
        attrs[.font] = NSFont.systemFont(ofSize: font.pointSize * 0.72, weight: .semibold)
        attrs[.foregroundColor] = NSColor.controlAccentColor
        attrs[.baselineOffset] = font.pointSize * 0.35
        attrs[Self.markdownSourceKey] = source
        attrs[Self.footnoteReferenceLabelKey] = label
        result.append(NSAttributedString(string: label, attributes: attrs))
    }

    private static func appendDoubleEqualsSeparator(
        font: NSFont,
        baseAttributes: [NSAttributedString.Key: Any],
        to result: NSMutableAttributedString
    ) {
        ...
        attrs[.font] = NSFont.systemFont(ofSize: font.pointSize * 0.85, weight: .medium)
    ...
    /// Parse inline footnote reference: [^label]. Does not match definitions: [^label]: text.
    private static func parseFootnoteReference(
        _ str: String,
        from start: String.Index
    ) -> (label: String, source: String, end: String.Index)? {
        let prefix = "[^"
        guard str[start...].hasPrefix(prefix) else { return nil }
        let labelStart = str.index(start, offsetBy: prefix.count)
        guard let closingBracket = str[labelStart...].firstIndex(of: "]") else { return nil }
        let end = str.index(after: closingBracket)
        if end < str.endIndex, str[end] == ":" {
            return nil
        }

        let label = String(str[labelStart..<closingBracket])
            .trimmingCharacters(in: .whitespacesAndNewlines)
        guard !label.isEmpty else { return nil }

        return (label, String(str[start..<end]), end)
    }
