...
private enum PageBlockParser {
    ...
    static func parseBlocks(_ markdown: String) -> [ParsedPageBlock] {
        ...
        while index < lines.count {
            ...
            if let headingLevel = parseToggleHeadingComment(trimmed) {
                ...
                while index < lines.count {
                    if lines[index].trimmingCharacters(in: .whitespaces) == "<!-- /toggle-heading -->" {
                        index += 1
                        break
                    }
    ...
    static func serializeDocument(
        ...
        style: PageMarkdownFormatStyle = .dahso
    ) -> String {
    ...
    private static func serializeChunk(
        ...
        if includeBlockIDComments, style == .dahso {
            lines.append("<!-- block-id: \(block.id) -->")
        }

        let hasColor = block.textColor != nil || block.backgroundColor != nil
        if hasColor, style == .dahso, block.type != .column, block.type != .toggle, block.type != .headingToggle {
            var parts: [String] = []
        ...
        case .databaseEmbed:
            ...
            case .commonmark:
                lines.append("**Dahso database:** \(pageDisplayName(fromPath: block.databasePath))")
            }
        ...
        case .headingToggle:
            ...
            case .commonmark:
                ...
                lines.append(block.isExpanded ? "<details open>" : "<details>")
                lines.append("<summary>\(hashes) \(escapeHTML(block.text))</summary>")
                if !block.children.isEmpty {
                    lines.append("")
                    lines.append(contentsOf: serializeLines(
                        block.children,
    ...
    private static func parsePageLinkComment(_ line: String) -> String? {
        let trimmed = line.trimmingCharacters(in: .whitespaces)
        guard trimmed.hasPrefix("<!--"), trimmed.hasSuffix("-->") else { return nil }
        let prefixes = ["dahso-page-link:", "page-link:"]
...
func previewWorkspacePageFormat(
    ...
    let persistBlockIDComments = style == .dahso && documentHasPersistedBlockIDs(document.blocks)
...
private func parsedPageBlockJSON(
    ...
    if !block.text.isEmpty || block.type == .paragraph || block.type == .heading || block.type == .blockquote || block.type == .toggle || block.type == .headingToggle {
        json["text"] = block.text
    }
    if block.headingLevel != 1 || block.type == .heading || block.type == .headingToggle {
