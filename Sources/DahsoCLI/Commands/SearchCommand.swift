...
private struct QmdBackend {
    ...
    var collectionName: String {
        ...
        return name.isEmpty ? "dahso" : name
    }
    ...
    private func ensureContextRegistered(run: ([String]) -> Void) {
        ...
        run(["context", "add", "qmd://\(collectionName)",
             "Dahso personal knowledge base — pages, databases, and meeting notes"])
        for db in databases {
    ...
    /// Map a qmd result object to DahsoCLI's search result format.
